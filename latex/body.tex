...
\begin{hebrew}פיגמנטים תזונתיים, כגון ה-\textenglish{betalains} המצויים בסלק, עשויים לצבוע באופן נראה את הצואה, ולגרום לתופעה הידועה בשם \textenglish{beeturia}. ה-\textenglish{betalains} הם פיגמנטים מסיסים במים המתפקדים ככרומופורים, ומשווים גוון אדום או סגול אופייני לצואה \cite{beeturia}. השפעה זו בולטת במיוחד בתנאים של הפרשת חומצה קיבתית מופחתת וצמחיית מעיים משתנה, ומדגישה את יחסי הגומלין המורכבים שבין הצריכה התזונתית לבין תפקוד מערכת העיכול. דוגמה אמיתית לתופעה זו מוצגת באיור~\ref{fig:F12}.\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F1.png}
\caption{\texthebrew{סלק נא ומבושל — מקור ה-\textenglish{betacyanin} הנחקר.}}\label{fig:F1}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=0.6\textwidth]{../assets/F12.png}
\caption{\texthebrew{דגימה אמיתית של צביעה צואתית במי האסלה לאחר צריכת סלק, המדגימה את גוון ה-\textenglish{beeturia} האדמדם-חום — המחשה מוקדמת של התופעה הנחקרת.}}\label{fig:F12}
\end{figure}
...
\begin{hebrew}מספר מחלקות של פיגמנטים תזונתיים ידועות כמשפיעות על צבע הצואה: \textenglish{betalains}, \textenglish{anthocyanins}, \textenglish{chlorophylls}, \textenglish{carotenoids} וצבעי \textenglish{azo}. לכל מחלקה תכונות כימיות נבדלות הקובעות את יכולתה לשרוד את המעבר במעי ולהקנות צבע לצואה \cite{delgado2000} (ראו איור~\ref{fig:F10}).\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F10.png}
\caption{\texthebrew{צביעה צואתית ייצוגית על פני פיגמנטים תזונתיים שונים.}}\label{fig:F10}
\end{figure}

\begin{hebrew}\textbf{\textenglish{Betalains}.} ה-\textenglish{betalains}, המצויים בעיקר בסלק (\textenglish{Beta vulgaris}) ובצמחים נוספים מסדרת ה-\textenglish{Caryophyllales}, הם תרכובות נגזרות-אינדול המאופיינות ב-\textenglish{betacyanins} אדומים-סגולים וב-\textenglish{betaxanthins} צהובים. ה-\textenglish{betacyanin} הנפוץ ביותר הוא ה-\textenglish{betanin}, המגלה יציבות פוטוכימית גבוהה אך יציבות תרמית בינונית \cite{stintzing2004}. מבנהו הכימי הייחודי מאפשר לו לעמוד בפני פירוק אנזימטי במערכת העיכול, ולכן הוא שורד אל הצואה ומקנה לה צביעה אדומה אופיינית.\end{hebrew}
...
\toprule
\textenglish{Food} & \textenglish{Pigment Class} & \textenglish{Chromophore} & \textenglish{Stool Coloration} & \textenglish{Onset (h)} & \textenglish{Duration (h)} & \textenglish{pH Stability} \\
\midrule
\textenglish{Beets} & \textenglish{Betalains} & \textenglish{Betanin} & \textenglish{Red} & \textenglish{4--6} & \textenglish{12--24} & \textenglish{Stable (pH$>$3)} \\
\textenglish{Berries} & \textenglish{Anthocyanins} & \textenglish{Anthocyanidins} & \textenglish{Purple/Red} & \textenglish{2--4} & \textenglish{8--16} & \textenglish{Variable} \\
\textenglish{Spinach} & \textenglish{Chlorophylls} & \textenglish{Phylloquinone} & \textenglish{Green} & \textenglish{2--3} & \textenglish{4--8} & \textenglish{Unstable} \\
\textenglish{Carrots} & \textenglish{Carotenoids} & \textenglish{Beta-carotene} & \textenglish{Orange/Yellow} & \textenglish{1--2} & \textenglish{6--12} & \textenglish{Stable} \\
\textenglish{Artificial} & \textenglish{Azo dyes} & \textenglish{Azo group} & \textenglish{Intense} & \textenglish{Var.} & \textenglish{Var.} & \textenglish{pH-dependent} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{llll}
\toprule
\textenglish{Bristol Type} & \textenglish{Description} & \textenglish{Typical Transit} & \textenglish{Water Content (\%)} \\
\midrule
\textenglish{1} & \textenglish{Separate hard lumps} & \textenglish{Long} & \textenglish{60} \\
\textenglish{2} & \textenglish{Sausage-shaped, lumpy} & \textenglish{Moderate} & \textenglish{70} \\
\textenglish{3} & \textenglish{Sausage with surface cracks} & \textenglish{Short--moderate} & \textenglish{75} \\
\textenglish{4} & \textenglish{Smooth, soft sausage} & \textenglish{Short} & \textenglish{80} \\
\textenglish{5} & \textenglish{Soft blobs, clear edges} & \textenglish{Short--moderate} & \textenglish{85} \\
\textenglish{6} & \textenglish{Mushy, ragged edges} & \textenglish{Moderate} & \textenglish{90} \\
\textenglish{7} & \textenglish{Watery, no solid pieces} & \textenglish{Very short} & \textenglish{$>$90} \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{\texthebrew{סולם הצואה של \textenglish{Bristol}: טיפוסי מרקם מול זמן מעבר.}}\label{fig:F7}
\end{figure}

\begin{hebrew}מדרג המרקם — מקשה ועד נוזלי — מסמן זמני שהייה לומינליים משתנים; צואה קשה (טיפוסים 1--2) מעידה על מעבר ממושך עקב תנועתיות מופחתת או ספיגת מים מוגברת \cite{heaton1992} (ראו איור~\ref{fig:F7}).\end{hebrew}
...
\begin{hebrew}תכולת המים היא גורם מכריע לצורת הצואה: תכולה גבוהה מתואמת עם צואה רכה (טיפוסים 5--7), ותכולה נמוכה — עם צואה קשה (טיפוסים 1--2). טיפוס 4 מייצג איזון אופטימלי בין הידרציה להתמצקות. דגימה אמיתית של צואה מוצקה אופיינית מוצגת באיור~\ref{fig:F14}.\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.42\textwidth]{../assets/F14.png}
\caption{\texthebrew{דגימה אמיתית של צואה מוצקה (\textenglish{Bristol} טיפוסים 3--4) על רקע ניטרלי, המדגימה צביעה חום-זיתנית עם אזורים אדמדמים האופייניים לשרידי פיגמנט \textenglish{betalain}.}}\label{fig:F14}
\end{figure}
...
\caption{\texthebrew{צינור המעבר במערכת העיכול: בליעה $\rightarrow$ קיבה $\rightarrow$ מעי דק $\rightarrow$ כרכשת $\rightarrow$ צואה, עם קבועי הקצב.}}\label{fig:D1}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F2.png}
\caption{\texthebrew{פרופילי ריכוז לומינלי $C(t)$ עבור \textenglish{betanin} מול \textenglish{anthocyanin} מול צבע \textenglish{azo}.}}\label{fig:F2}
\end{figure}

\begin{hebrew}תרשים צינור המעבר מוצג באיור~\ref{fig:D1}, ופרופילי הריכוז החזויים מוצגים באיור~\ref{fig:F2}. מסגרת זו מספקת בסיס להבנת האופן שבו פיגמנטים תזונתיים כגון ה-\textenglish{betalains} משפיעים על מאפייני הצואה, ומציעה תובנות ליישומים אבחוניים או טיפוליים הקשורים לבריאות המעי.\end{hebrew}
...
\begin{hebrew}להערכת ההפרש הנתפס בין שני צבעים בדגימות צואה משתמשים בנוסחת \textenglish{CIEDE2000} \cite{luo2001}, המביאה בחשבון הפרשי גוון ($\Delta H'$), בהירות ($\Delta L'$) וכרומה ($\Delta C'$), וכן פונקציות שִקלול אדפטיביות. הביטוי המרכזי:\end{hebrew}
...
\begin{hebrew}מספר שיטות משמשות למדידה כמותית של צבע הצואה: ניתוח ספקטרופוטומטרי, הערכה חזותית, ודימות דיגיטלי. לכל אחת יתרונות ומגבלות מבחינת דיוק, עלות ומהימנות בין-מעריכים. מערך הדגימה הספקטרופוטומטרי מתואר באיור~\ref{fig:D2}, ולוח ייחוס הצבע באיור~\ref{fig:F3}.\end{hebrew}

\begin{table}[h]\centering\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{llll}
\toprule
\textenglish{Method} & \textenglish{Quantitative?} & \textenglish{Cost} & \textenglish{Inter-rater Reliability} \\
\midrule
\textenglish{Spectrophotometry} & \textenglish{Yes} & \textenglish{High} & \textenglish{High} \\
\textenglish{Visual (Bristol-adapted)} & \textenglish{No} & \textenglish{Low} & \textenglish{Moderate} \\
\textenglish{Digital Imaging} & \textenglish{Yes} & \textenglish{Medium} & \textenglish{High} \\
\bottomrule
\end{tabular}
\end{table}
...
\end{tikzpicture}
\caption{\texthebrew{מערך דגימה ספקטרופוטומטרי לצואה.}}\label{fig:D2}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F3.png}
\caption{\texthebrew{לוח ייחוס צבע \textenglish{CIELAB} ($a^*$) לדירוג צביעה צואתית.}}\label{fig:F3}
\end{figure}

\begin{hebrew}לסיכום, מרחב \textenglish{CIELAB} ונוסחת \textenglish{CIEDE2000} מספקים מסגרת איתנה לכימות שינויי צבע הצואה \cite{mokrzycki2011}. שיטות ספקטרופוטומטריות מציעות דיוק גבוה אך יקרות, בעוד דימות דיגיטלי מאזן בין דיוק לעלות.\end{hebrew}
...
\begin{hebrew}ככל שה-\textenglish{pH} הצואתי יורד, היחס $[A^-]/[HA]$ עולה, ומכאן שיעור גבוה יותר של \textenglish{betanin} אדום ועוצמת צבע מוגברת, כפי שמוצג באיור~\ref{fig:F8}. קשר זה מסביר מדוע צרכני סלק ומזונות עתירי \textenglish{betalain} מציגים לעיתים קרובות צביעה אדומה חזקה של הצואה (\textenglish{beeturia}).\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F8.png}
\caption{\texthebrew{חומציות צואתית מול שימור האדום ($a^*$) של ה-\textenglish{betalain} בצואה.}}\label{fig:F8}
\end{figure}

\begin{hebrew}טווח ה-\textenglish{pH} הצואתי האנושי נע בדרך כלל בין $5.5$ ל-$7.5$. בתוך טווח זה, מצב הפרוטונציה מווסת דינמית על ידי הצריכה התזונתית ופעילות המיקרוביום: סיבים ופחמימות מותססות משנים את ה-\textenglish{pH} הכרכשתי דרך תסיסה מיקרוביאלית, ובכך משפיעים גם על זמני המעבר ועל מרקם הצואה \cite{macfarlane2003, topping2001}.\end{hebrew}
...
\begin{tabular}{lllll}
\toprule
\textenglish{Component} & \textenglish{Symbol} & \textenglish{Range} & \textenglish{Weight} & \textenglish{Rationale} \\
\midrule
\textenglish{Colour difference} & \textenglish{$\hat{E}$} & \textenglish{[0, 1]} & \textenglish{$w_1$} & \textenglish{chromatic deviation from the reference standard} \\
\textenglish{BSS score} & \textenglish{$\hat{B}$} & \textenglish{[0, 1]} & \textenglish{$w_2$} & \textenglish{stool consistency on a normalized scale} \\
\textenglish{Faecal pH} & \textenglish{$\widehat{\mathrm{pH}}$} & \textenglish{[0, 1]} & \textenglish{$w_3$} & \textenglish{digestive health and metabolic state} \\
\textenglish{Luminal transit} & \textenglish{$\hat{T}$} & \textenglish{[0, 1]} & \textenglish{$w_4$} & \textenglish{transit-time efficiency of digestion} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{hebrew}לכל דגימה נמדדו ארבעה פרמטרים: (1) \textbf{צבע} — ניתוח \textenglish{CIEDE2000} באמצעות ספקטרופוטומטר; (2) \textbf{מרקם} — סיווג לפי \textenglish{Bristol Stool Scale} (טיפוסים 1--7); (3) \textbf{חומציות} — מדידת \textenglish{pH} במד-\textenglish{pH} נייד; (4) \textbf{זמן מעבר} — הערכה בשיטת סמנים (\textenglish{markers}) \cite{cummings1976, metcalf1987}.\end{hebrew}
...
\begin{hebrew}המודל שלנו מצביע כי צריכת \textenglish{betalain} תזונתית משפיעה באופן מובהק על צביעת הצואה, עם שונות ניכרת בין תת-קבוצות לפי גיל ומגדר. ערך ה-$\Delta E$ הממוצע בקבוצה הצעירה (מתחת ל-30) היה $15.7 \pm 3.8$, לעומת $9.4 \pm 2.6$ במבוגרים (מעל 60), $p < 0.001$. נשים הציגו שכיחות גבוהה יותר של \textenglish{beeturia} בכל קבוצות הגיל (ראו איור~\ref{fig:F4}). דגימה אמיתית של צביעה צואתית עזה לאחר צריכת סלק מוצגת באיור~\ref{fig:F13}.\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.5\textwidth]{../assets/F13.png}
\caption{\texthebrew{דגימה אמיתית של צביעה צואתית עזה לאחר צריכת סלק: פירוק ניכר של פיגמנט ה-\textenglish{betalain} צובע את מי האסלה בגוון ורוד-אדום חי (\textenglish{beeturia}), המתאים לערכי $\Delta E$ גבוהים.}}\label{fig:F13}
\end{figure}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F4.png}
\caption{\texthebrew{שכיחות הפיגמנטציה הצואתית על פני תת-קבוצות.}}\label{fig:F4}
\end{figure}

\subsection{\texthebrew{$\Delta E$ כפונקציית מנה}}

\begin{hebrew}הקשר בין מנת ה-\textenglish{betalain} לצביעת הצואה מתואר היטב בעקומה סיגמואידלית $\Delta E = A/[1 + e^{-k(x - x_0)}]$, כאשר $A$ עוצמת הצבע המרבית, $x_0$ מנת נקודת הפיתול ו-$k$ תלילות העקומה. למבוגר טיפוסי (70 ק"ג), צריכה של 2 מ"ג/ק"ג הניבה $\Delta E$ ממוצע של $18.3 \pm 4.5$ (ראו איור~\ref{fig:F5}).\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F5.png}
\caption{\texthebrew{עקומת ה-$\Delta E$ הסיגמואידלית כפונקציה של מנת הסלק.}}\label{fig:F5}
\end{figure}

\subsection{\texthebrew{זמן מעבר והתפלגות טיפוסי \textenglish{Bristol}}}

\begin{hebrew}זמן המעבר הלומינלי נמצא במתאם הפוך עם מרקם הצואה לפי \textenglish{Bristol Stool Scale}: משתתפים בטיפוסים 2--3 הציגו זמן מעבר ממוצע של $10 \pm 2$ שעות, לעומת $4.8 \pm 1.2$ שעות בטיפוסים 5--6 ($p < 0.001$); התפלגות זמני המעבר מוצגת באיור~\ref{fig:F6}.\end{hebrew}

\begin{figure}[h]\centering
\includegraphics[width=0.9\textwidth]{../assets/F6.png}
\caption{\texthebrew{התפלגות זמן המעבר במערכת העיכול עבור פיגמנט הצואה.}}\label{fig:F6}
\end{figure}
...
\begin{hebrew}מטריצת המתאמים מציגה את מקדם המתאם של \textenglish{Pearson} בין כל זוג פרמטרים, ובכך מקלה על הבנת האופן שבו שינוי במשתנה אחד מתואם עם שינוי באחר (ראו איור~\ref{fig:F9}). המתאמים שנצפו הם כדלקמן:\end{hebrew}
...
\caption{\texthebrew{מטריצת מתאמים בין פרמטרי הצואה ($\Delta E$, \textenglish{Bristol}, \textenglish{pH}, מעבר, \textenglish{FCI}).}}\label{fig:F9}
\end{figure}
...
\begin{hebrew}מחקר המשך צריך להתמקד בפיתוח מודלים של על-הצבה רב-פיגמנטית (\textenglish{multi-pigment superposition}) המביאים בחשבון את האינטראקציות בין כרומופורים תזונתיים שונים, ובכך לשפר את פרשנות הצביעה הצואתית ואת ההבחנה בין השפעות תזונתיות חולפות לבעיות בריאות כרוניות \cite{clarke2014}.\end{hebrew}

\begin{hebrew}יתרה מזאת, מסווגי למידת מכונה (\textenglish{machine learning classifiers}) שאומנו על מאגרי דגימות צואה גדולים עשויים לאפשר שחזור דפוסי צריכה תזונתית על בסיס מאפייני הצואה בלבד, ובכך לאפשר ייעוץ תזונתי מותאם אישית \cite{zeevi2015, berry2020} וזיהוי מוקדם של הפרעות עיכול.\end{hebrew}
